...
var rUseOptions = new RegExp('\\/v' + VERSION + '\\/(\\S+)\\?(\\S+)?$')
  , rNotOptions = new RegExp('\\/v' + VERSION + '\\/(\\S+)')
  , rQueryStr = /\?(\S+)$/;
...
var Vectogram = function (opts) {
  this.server = http.createServer(function (req, res) {
    ...
    var useOptions = rUseOptions.exec(req.url)
      , notOptions = rNotOptions.exec(req.url);

    var match = useOptions ? useOptions : notOptions;
    ...
    var imgUrl = match[1]
      ...
      , options = query ? querystring.parse(query[1]): {};
